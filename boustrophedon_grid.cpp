...
This is a discrete version of the boustrophedon idea:
...
static int createCell(std::vector<DecompositionCell>& cells) {
    ...
    cells.push_back({id, {}, {}, false, false});
    return id;
}
...
static std::vector<int> orderedNeighbors(const std::vector<DecompositionCell>& cells, int cell_id) {
    std::vector<int> neighbors(cells[cell_id].neighbors.begin(), cells[cell_id].neighbors.end());

    double center_row = 0.0;
    double center_col = 0.0;
    for (Point p : cells[cell_id].points) {
        center_row += p.row;
        center_col += p.col;
    }
    center_row /= cells[cell_id].points.size();
    center_col /= cells[cell_id].points.size();

    std::sort(neighbors.begin(), neighbors.end(), [&](int a, int b) {
        auto angleFor = [&](int id) {
            double row = 0.0;
            double col = 0.0;
            for (Point p : cells[id].points) {
                row += p.row;
                col += p.col;
            }
            row /= cells[id].points.size();
            col /= cells[id].points.size();
            return std::atan2(row - center_row, col - center_col);
        };
...
static void buildDfsWalk(std::vector<DecompositionCell>& cells,
                         int cell_id,
                         std::vector<int>& path) {
    cells[cell_id].visited = true;
    path.push_back(cell_id);

    for (int neighbor_id : orderedNeighbors(cells, cell_id)) {
        if (cells[neighbor_id].visited) {
            continue;
        }

        buildDfsWalk(cells, neighbor_id, path);
        path.push_back(cell_id);
    }
}
...
static std::vector<Point> buildBfsPath(
    ...
    while (!frontier.empty()) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (allowed != nullptr && allowed->count({next.row, next.col}) == 0 &&
                !samePoint(next, goal)) {
                continue;
            }
...
static std::set<std::pair<int, int>> pointSetForCell(const DecompositionCell& cell) {
    std::set<std::pair<int, int>> points;
    for (Point p : cell.points) {
        points.insert({p.row, p.col});
    }
    return points;
}
...
static void moveToCell(CoverageResult& result,
                       ...
                       const std::vector<std::string>& grid,
                       std::set<std::pair<int, int>>& covered) {
    std::set<std::pair<int, int>> cell_points = pointSetForCell(cell);
    if (cell_points.count({current.row, current.col}) > 0) {
        return;
    }

    Point entry = nearestPointInCell(current, cell);
    std::vector<Point> path = buildBfsPath(current, entry, rows, cols, grid, nullptr);
    appendPath(result, path, covered, current);
}

static void cleanCell(CoverageResult& result,
                      Point& current,
                      DecompositionCell& cell,
                      int rows,
                      int cols,
                      const std::vector<std::string>& grid,
                      std::set<std::pair<int, int>>& covered) {
    ...
    for (Point target : sweep) {
        ...
        std::vector<Point> path = buildBfsPath(current, target, rows, cols, grid, &cell_points);
        if (path.empty() && !samePoint(current, target)) {
            path = buildBfsPath(current, target, rows, cols, grid, nullptr);
        }
...
CoverageResult runBoustrophedonGridTraversal(int rows, int cols, Point start,
    ...
    buildDfsWalk(cells, start_cell, cell_walk);
